...
Cadastro de tenant via UI (antes só acessível via cURL direto em `POST /auth/register-tenant`) e login que não exige `tenantSlug` no caso comum (usuário com um único tenant).

Feature nova — sem RN legada correspondente. Traceability map: `docs/migration/rn-to-openspec-map.md`.
