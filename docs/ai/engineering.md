...
1. Leia o sprint ativo em `docs/sprints/`
2. Leia o **Plano de Execução** da sprint — identifique os Sets e a ordem
3. Leia todas as RNs listadas na sprint em `docs/business-rules/`
4. Consulte `docs/context/project.md` para stack e convenções
5. Consulte `docs/decisions/` para áreas com decisões arquiteturais registradas
6. Nunca assuma comportamento não documentado — aponte a lacuna antes de prosseguir
7. Em análise de arquivos grandes, aplique a skill `dont-be-greedy` para leitura incremental
8. Confirme que o documento da sprint possui a seção `Testes Manuais de Entrega (Passo a Passo Executável)`; se não houver, complete essa seção antes de iniciar execução

### 1.1 Seção obrigatória no documento de entrega da sprint

Todo documento de sprint deve conter a seção:

`## Testes Manuais de Entrega (Passo a Passo Executável)`

Essa seção deve ser prática e executável, contendo no mínimo:

- cenário de teste e objetivo;
- pré-requisitos explícitos;
- passos numerados com comando/ação exata;
- resultado esperado em cada passo;
- critério final de aprovação da entrega.

Sem essa seção preenchida, a sprint não pode ser considerada entregue.
...
RN > padrão técnico > convenção de código. Nunca quebre uma RN para simplificar.
...
As skills abaixo implementam workflows completos. Invoque pelo nome quando solicitado:

| Skill | Quando usar |
|---|---|
| `sprint-set-execution` | Implementar/continuar/revisar sets de uma sprint |
| `sprint-definition-rn-flow` | Planejar sprint, definir escopo, organizar RNs |
| `code-review` | Code review estruturado (chamada automaticamente pelo fluxo) |
| `refactor-pass` | Passagem de refatoração ao final da sprint (automática) |
| `git-workflow` | Garantir consistência de versionamento e PRs |
| `dont-be-greedy` | Controlar consumo de contexto em arquivos grandes |
| `sprint-to-changelog` | Gerar changelog ao final da sprint (automática) |
| `text-to-business-rule` | Converter texto livre em RN estruturada |

Definições completas em `docs/ai/skills/`.
